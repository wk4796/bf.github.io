...
# =================================================================
#      图片画廊 极致优化版 - 一体化部署与管理脚本 (v8.5)
#
#   作者: 编码助手
#   功能: 全面优化前端UI和交互，修复分类、上传及菜单显示问题。
# =================================================================
...
cat << 'EOF' > package.json
{
  ...
  "version": "8.5.0",
  "description": "A robust, scalable, and secure full-stack image gallery application with fully optimized UI and enhanced compatibility.",
  "main": "server.js",
  "scripts": {
...
const { db } = require('../../data/database.js'); const { body, validationResult } = require('express-validator');
exports.getPublicCategories = (req, res) => { try { const query = `SELECT c.id, c.name FROM categories c WHERE EXISTS (SELECT 1 FROM images i WHERE i.category_id = c.id) ORDER BY CASE WHEN c.name = '未分类' THEN 0 ELSE 1 END, c.name COLLATE NOCASE`; res.json(db.prepare(query).all()); } catch (e) { res.status(500).json({ message: 'Error fetching categories' }); } };
exports.getAllCategories = (req, res) => { try { const query = ` SELECT c.id, c.name, COUNT(i.id) as image_count FROM categories c LEFT JOIN images i ON c.id = i.category_id GROUP BY c.id, c.name ORDER BY CASE WHEN c.name = '未分类' THEN 0 ELSE 1 END, c.name COLLATE NOCASE `; res.json(db.prepare(query).all()); } catch (e) { console.error(e); res.status(500).json({ message: 'Error fetching categories for admin' }); } };
exports.createCategory = [ body('name').trim().notEmpty().isLength({ min: 1, max: 50 }), (req, res) => { if (!validationResult(req).isEmpty()) return res.status(400).json({ message: 'Invalid category name' }); try { const info = db.prepare('INSERT INTO categories (name) VALUES (?)').run(req.body.name); res.status(201).json({ id: info.lastInsertRowid, name: req.body.name }); } catch (e) { res.status(e.code === 'SQLITE_CONSTRAINT_UNIQUE' ? 409 : 500).json({ message: 'Category exists or server error' }); } }];
exports.updateCategory = [ body('name').trim().notEmpty().isLength({ min: 1, max: 50 }), (req, res) => { if (!validationResult(req).isEmpty()) return res.status(400).json({ message: 'Invalid category name' }); const cat = db.prepare('SELECT name FROM categories WHERE id = ?').get(req.params.id); if (!cat) return res.status(404).json({ message: 'Category not found' }); if (cat.name === '未分类') return res.status(403).json({ message: 'Cannot rename "未分类"' }); try { db.prepare('UPDATE categories SET name = ? WHERE id = ?').run(req.body.name, req.params.id); res.json({ id: Number(req.params.id), name: req.body.name }); } catch (e) { res.status(e.code === 'SQLITE_CONSTRAINT_UNIQUE' ? 409 : 500).json({ message: 'Category name exists or server error' }); } }];
exports.deleteCategory = (req, res) => { const cat = db.prepare('SELECT name FROM categories WHERE id = ?').get(req.params.id); if (!cat) return res.status(404).json({ message: 'Category not found' }); if (cat.name === '未分类') return res.status(403).json({ message: 'Cannot delete "未分类"' }); try { const uncategorized = db.prepare(`SELECT id FROM categories WHERE name = '未分类'`).get(); if (!uncategorized) throw new Error('Default category missing'); db.transaction(() => { db.prepare('UPDATE images SET category_id = ? WHERE category_id = ?').run(uncategorized.id, req.params.id); db.prepare('DELETE FROM categories WHERE id = ?').run(req.params.id); })(); res.status(200).json({ message: 'Category deleted' }); } catch (e) { res.status(500).json({ message: 'Error deleting category' }); } };
EOF
...
const queryImages = (req, res, isAdmin = false) => {
    ...
    try {
        const totalResult = db.prepare(`SELECT COUNT(*) as total FROM images i JOIN categories c ON i.category_id = c.id ${whereClause}`).get(...params);
        const total = totalResult ? totalResult.total : 0;
        const images = db.prepare(`${getBaseQuery()} ${whereClause} ${orderBy} LIMIT ? OFFSET ?`).all(...params, limit, (page - 1) * limit);
        res.json({ images, totalPages: Math.ceil(total / limit), currentPage: Number(page), totalItems: total });
    } catch (e) { console.error("Query images error:", e); res.status(500).json({ message: 'Error fetching images' }); }
};
...
exports.uploadImage = (req, res) => {
    upload(req, res, async (err) => {
        ...
        try {
            ...
            db.prepare(`INSERT INTO images (id, category_id, description, filename_orig, size_orig, path_orig, path_display, path_thumb) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).run(imageId, req.body.category_id, req.body.description || null, req.file.originalname, req.file.size, paths.path_orig, paths.path_display, paths.path_thumb);
            res.status(201).json({ message: 'Upload successful', imageId: imageId });
...
EOF
    ...
    echo "--> [v8.5] 正在重构后台管理页 public/admin.html (全面优化UI和交互)..."
cat << 'EOF' > public/admin.html
<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>后台管理 - 图片画廊</title><script src="https://cdn.tailwindcss.com"></script><style>body{background-color:#f3f4f6}.category-item.active{background-color:#eef2ff;font-weight:600;color:#4f46e5}.upload-drop-zone{transition:background-color .2s ease}.upload-drop-zone.dragover{background-color:#eef2ff;border-style:solid}dialog::backdrop{background-color:rgba(0,0,0,.5)}</style></head><body class="bg-gray-100 text-gray-800 font-sans"><header class="bg-white shadow-md p-4 flex justify-between items-center sticky top-0 z-30"><h1 class="text-xl sm:text-2xl font-semibold text-indigo-700">内容管理</h1><div><a href="/" target="_blank" class="text-indigo-600 hover:text-indigo-800 mr-2 sm:mr-4">查看前台</a><a href="/api/auth/logout" class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-3 sm:px-4 rounded transition-colors text-sm sm:text-base">退出</a></div></header><main class="p-4 sm:p-6 lg:p-8"><div class="grid grid-cols-1 lg:grid-cols-12 gap-8"><aside class="lg:col-span-4 xl:col-span-3 space-y-6"><section class="bg-white p-6 rounded-lg shadow-md"><h2 class="text-lg font-semibold text-gray-900 mb-4">上传新图片</h2><form id="upload-form" class="space-y-4"><div id="upload-drop-zone" class="w-full flex flex-col items-center justify-center p-4 border-2 border-dashed border-gray-400 rounded-lg cursor-pointer hover:bg-gray-100"><svg class="w-10 h-10 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6 9 9 0 0016 13.097m-1-14.194l-2-2-2 2m-2-2l2 2 2-2m2 2l-2-2-2 2m7-2a9 9 0 019 9m-9-9l2 2 2-2m-2-2l-2 2-2-2"></path></svg><p class="mt-2 text-sm text-gray-600"><span class="font-semibold">点击选择</span> 或拖拽文件到这里</p><input id="image-input" type="file" class="hidden" multiple accept="image/png,image/jpeg,image/gif,image/webp"></div><div id="upload-queue-wrapper" class="hidden"><div id="upload-queue-list" class="space-y-2 max-h-40 overflow-y-auto border-t border-b py-2 my-2"></div><div class="flex justify-between items-center text-sm font-medium text-gray-600"><p>总计: <span id="queue-total-count">0</span> 张, <span id="queue-total-size">0 KB</span></p><button type="button" id="clear-queue-btn" class="text-red-500 hover:underline">清空队列</button></div></div><div><label for="category-select" class="block text-sm font-medium text-gray-700 mb-1">上传到分类</label><select id="category-select" class="w-full border rounded px-3 py-2 text-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-500"></select></div><div><label for="description" class="block text-sm font-medium text-gray-700 mb-1">统一描述 (可选)</label><input type="text" id="description" class="w-full border rounded px-3 py-2 text-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-500" placeholder="应用于本次上传的所有图片"></div><button type="submit" id="upload-btn" class="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 px-4 rounded-lg transition-colors disabled:bg-gray-400">开始上传</button><p id="upload-progress" class="text-center text-sm font-medium text-indigo-700 h-5"></p></form></section><section class="bg-white p-6 rounded-lg shadow-md"><h2 class="text-lg font-semibold text-gray-900 mb-4">分类管理</h2><div class="flex items-center space-x-2 mb-4"><input type="text" id="new-category-name" class="w-full border rounded px-3 py-2 text-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-500" placeholder="新分类名称"><button id="add-category-btn" class="flex-shrink-0 bg-indigo-500 hover:bg-indigo-600 text-white font-bold w-9 h-9 rounded-full flex items-center justify-center text-xl">+</button></div><div id="category-management-list" class="space-y-1 max-h-72 overflow-y-auto"></div></section></aside><section class="bg-white p-6 rounded-lg shadow-md lg:col-span-8 xl:col-span-9"><div class="flex flex-wrap justify-between items-center gap-4 mb-4"><h2 class="text-lg font-semibold text-gray-900">已上传图片 <span id="image-count" class="text-sm text-gray-500 font-normal"></span></h2><div class="relative min-w-0 flex-grow sm:flex-grow-0 sm:w-64"><input type="search" id="search-input" class="w-full pl-10 pr-4 py-2 text-sm border rounded-full text-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-500" placeholder="搜索图片..."><div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none"><svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg></div></div></div><div id="image-list" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-3 xl:grid-cols-4 gap-6"></div><div id="pagination-controls" class="flex justify-center items-center space-x-4 mt-6"></div></section></div></main><dialog id="edit-image-dialog" class="p-6 rounded-lg shadow-xl w-full max-w-md"><h3 class="text-xl font-semibold text-gray-900 mb-4">编辑图片信息</h3><form id="edit-image-form" class="space-y-4"><input type="hidden" id="edit-id"><img id="edit-preview" class="w-full h-48 object-cover rounded-md bg-gray-200 mb-4"><div><label for="edit-category-select" class="block text-sm font-medium text-gray-700 mb-1">分类</label><select id="edit-category-select" class="w-full border rounded px-3 py-2 text-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-500"></select></div><div><label for="edit-description" class="block text-sm font-medium text-gray-700 mb-1">描述</label><input type="text" id="edit-description" class="w-full border rounded px-3 py-2 text-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-500"></div><div class="mt-4 pt-4 border-t text-sm text-gray-600 space-y-1"><p><strong>文件名:</strong> <span id="edit-info-filename"></span></p><p><strong>文件大小:</strong> <span id="edit-info-size"></span></p><p><strong>上传日期:</strong> <span id="edit-info-date"></span></p></div><div class="flex justify-end space-x-2 mt-6"><button type="button" id="cancel-edit-btn" class="bg-gray-300 hover:bg-gray-400 text-gray-800 py-2 px-4 rounded font-semibold">取消</button><button type="submit" class="bg-indigo-600 hover:bg-indigo-700 text-white py-2 px-4 rounded font-semibold">保存</button></div></form></dialog>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const util = {
        ...
        showToast: (message, isError = false) => { const toast = document.createElement('div'); toast.textContent = message; toast.className = `fixed top-5 right-5 p-4 rounded-lg shadow-lg text-white ${isError ? 'bg-red-500' : 'bg-indigo-600'} z-50`; document.body.appendChild(toast); setTimeout(() => toast.remove(), 3000); },
    ...
    // --- STATE & DOM ELEMENTS ---
    let state = { categories: [], currentPage: 1, totalItems: 0, filterCategory: 'all', search: '', uploadQueue: [] };
    const elements = {
    ...
    const api = async (url, options = {}) => {
        try {
            ...
            if (!response.ok) { util.showToast(data.message || `Error: ${response.status}`, true); return null; }
            return data;
        } catch (error) { util.showToast('Network error', true); return null; }
    };
    ...
    const loadCategories = async () => {
        ...
        if (categories) {
            ...
            const selects = [elements.upload.categorySelect, elements.editDialog.category];
            selects.forEach(select => {
                select.innerHTML = '';
                state.categories.forEach(cat => {
                    const option = new Option(`${cat.name} (${cat.image_count})`, cat.id);
                    select.add(option);
                });
            });
    ...
    const loadImages = async () => {
        const params = new URLSearchParams({ page: state.currentPage, limit: 12, category: state.filterCategory, search: state.search });
        const data = await api(`/api/admin/images?${params.toString()}`);
        if (data) {
            renderImageList(data.images);
            renderPagination(data.currentPage, data.totalPages);
            state.totalItems = data.totalItems;
            elements.imageCount.textContent = `(共 ${state.totalItems} 项)`;
        }
    };
    ...
    const handleFiles = (files) => {
        [...files].forEach(file => {
            if (file.type.startsWith('image/') && !state.uploadQueue.some(f => f.name === file.name && f.size === file.size)) {
    ...
    const renderUploadQueue = () => {
        elements.upload.queueWrapper.classList.toggle('hidden', state.uploadQueue.length === 0);
        elements.upload.queueList.innerHTML = state.uploadQueue.map((file, index) =>
            `<div class="flex items-center justify-between text-sm p-1 bg-gray-50 rounded">
                <span class="truncate w-4/6" title="${file.name}">${file.name}</span>
                <span class="text-gray-500">${util.formatBytes(file.size)}</span>
                <button type="button" data-index="${index}" class="remove-queue-item-btn text-red-500 font-bold text-lg leading-none">&times;</button>
            </div>`
        ).join('');
        const totalSize = state.uploadQueue.reduce((acc, file) => acc + file.size, 0);
        elements.upload.totalCount.textContent = state.uploadQueue.length;
        elements.upload.totalSize.textContent = util.formatBytes(totalSize);
    };
    
    const uploadNextFile = async () => {
        if (state.uploadQueue.length === 0) {
            util.showToast('全部上传完成！');
            elements.upload.progress.textContent = '';
            elements.upload.btn.disabled = false;
            await Promise.all([loadImages(), loadCategories()]);
            return;
        }
        const initialCount = parseInt(elements.upload.totalCount.textContent) || state.uploadQueue.length;
        elements.upload.progress.textContent = `正在上传第 ${initialCount - state.uploadQueue.length + 1} / ${initialCount} 张...`;
        
        const file = state.uploadQueue[0];
        const formData = new FormData();
        formData.append('image', file);
        formData.append('category_id', elements.upload.categorySelect.value);
        formData.append('description', elements.upload.description.value);

        const result = await api('/api/admin/images/upload', { method: 'POST', body: formData });
        if (!result) util.showToast(`"${file.name}" 上传失败`, true);
    ...
    const renderCategoryManagementList = () => {
        ...
        state.categories.forEach(cat => {
            ...
            item.className = `category-item flex items-center justify-between p-2 rounded-md ${state.filterCategory == cat.id ? 'active' : 'hover:bg-gray-100'}`;
            item.dataset.id = cat.id;
            item.dataset.name = cat.name;

            item.innerHTML = `
                <div class="flex-grow flex items-center cursor-pointer category-name-wrapper">
                    <span class="category-name font-medium">${cat.name}</span>
                ...
                ${!isUncategorized ? `<div class="space-x-2 flex-shrink-0 category-actions">
                    <button class="rename-cat-btn text-blue-500 hover:text-blue-700 text-sm font-semibold">重命名</button>
                    <button class="delete-cat-btn text-red-500 hover:text-red-700 text-sm font-semibold">删除</button>
                </div>` : ''}`;
    ...
    const renderImageList = (images) => {
        elements.imageList.innerHTML = images.length > 0 ? images.map(img => `
            <div class="bg-white rounded-lg shadow overflow-hidden group">
                <img src="${img.path_thumb}" alt="${img.description}" class="w-full h-40 object-cover">
                <div class="p-3 space-y-2">
                    <p class="font-semibold text-sm truncate" title="${img.filename_orig}">${img.filename_orig}</p>
                    <p class="text-xs text-gray-600">${img.category_name}</p>
                    <p class="text-xs text-gray-500">${util.formatBytes(img.size_orig)} - ${util.formatDate(img.created_at).split(' ')[0]}</p>
                    <div class="flex items-center justify-end space-x-2 pt-2">
                        <button data-image='${JSON.stringify(img)}' class="edit-btn text-white bg-blue-500 hover:bg-blue-600 rounded px-3 py-1.5 text-sm font-semibold">修改</button>
                        <button data-id="${img.id}" class="delete-btn text-white bg-red-500 hover:bg-red-600 rounded px-3 py-1.5 text-sm font-semibold">删除</button>
                    </div>
                </div>
            </div>`).join('') : '<p class="text-center text-gray-500 col-span-full">该分类下没有图片。</p>';
    };
    ...
    const renderPagination = (currentPage, totalPages) => {
        ...
        let buttons = [];
        buttons.push(`<button data-page="${currentPage - 1}" ${currentPage === 1 ? 'disabled' : ''} class="px-3 py-1 rounded font-semibold bg-white border border-gray-300 disabled:opacity-50">&laquo;</button>`);
        for (let i = 1; i <= totalPages; i++) {
            if (i === currentPage) {
                buttons.push(`<button data-page="${i}" class="px-3 py-1 rounded bg-indigo-600 text-white font-semibold border border-indigo-600">${i}</button>`);
            } else if (i <= 2 || i >= totalPages - 1 || Math.abs(i - currentPage) <= 1) {
                buttons.push(`<button data-page="${i}" class="px-3 py-1 rounded bg-white border border-gray-300 font-semibold">${i}</button>`);
            } else if (Math.abs(i - currentPage) === 2) {
                 buttons.push(`<span class="px-3 py-1">...</span>`);
            }
        }
        buttons.push(`<button data-page="${currentPage + 1}" ${currentPage === totalPages ? 'disabled' : ''} class="px-3 py-1 rounded font-semibold bg-white border border-gray-300 disabled:opacity-50">&raquo;</button>`);
        pagination.innerHTML = buttons.join('');
    };

    // --- EVENT LISTENERS ---
    const setupEventListeners = () => {
        elements.upload.dropZone.addEventListener('click', () => elements.upload.input.click());
        elements.upload.dropZone.addEventListener('dragover', e => { e.preventDefault(); e.currentTarget.classList.add('dragover'); });
        elements.upload.dropZone.addEventListener('dragleave', e => e.currentTarget.classList.remove('dragover'));
        elements.upload.dropZone.addEventListener('drop', e => { e.preventDefault(); e.currentTarget.classList.remove('dragover'); handleFiles(e.dataTransfer.files); });
        elements.upload.input.addEventListener('change', e => handleFiles(e.target.files));
        elements.upload.queueList.addEventListener('click', e => { if (e.target.classList.contains('remove-queue-item-btn')) { state.uploadQueue.splice(e.target.dataset.index, 1); renderUploadQueue(); } });
        elements.upload.clearQueueBtn.addEventListener('click', () => { state.uploadQueue = []; renderUploadQueue(); });
        ...
        elements.categoryMgmt.list.addEventListener('click', async e => {
            const item = e.target.closest('.category-item'); if (!item) return;
            ...
            if (currentlyEditing && currentlyEditing !== item) renderCategoryManagementList();

            if (e.target.classList.contains('rename-cat-btn')) {
                item.dataset.editing = 'true';
                item.querySelector('.category-name-wrapper').innerHTML = `<input type="text" class="cat-edit-input w-full border rounded px-2 py-1 text-sm focus:ring-2 focus:ring-indigo-500" value="${name}">`;
                item.querySelector('.category-actions').innerHTML = `<button class="save-cat-btn text-green-600 hover:text-green-800 text-sm font-semibold">保存</button><button class="cancel-edit-btn text-gray-500 hover:text-gray-700 text-sm font-semibold">取消</button>`;
                ...
                if (newName && newName !== name) { if (await api(`/api/admin/categories/${id}`, { method: 'PUT', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ name: newName }) })) { util.showToast('分类已重命名'); await loadCategories(); }
                } else { renderCategoryManagementList(); }
            } else if (e.target.classList.contains('cancel-edit-btn')) {
                renderCategoryManagementList();
            } else if (e.target.classList.contains('delete-cat-btn')) {
                if (confirm(`确定删除分类 "${name}"? 相关图片将移至"未分类"。`)) { if (await api(`/api/admin/categories/${id}`, { method: 'DELETE' })) { util.showToast('分类已删除'); if (state.filterCategory == id) state.filterCategory = 'all'; await Promise.all([loadCategories(), loadImages()]); } }
            } else if (e.target.closest('.category-name-wrapper')) {
                state.filterCategory = state.filterCategory == id ? 'all' : id; state.currentPage = 1; renderCategoryManagementList(); await loadImages();
            }
        });
        ...
        elements.imageList.addEventListener('click', e => {
            ...
            if (editBtn) {
                const img = JSON.parse(editBtn.dataset.image); const dialog = elements.editDialog;
                dialog.id.value = img.id; dialog.preview.src = img.path_thumb; dialog.category.value = img.category_id;
                dialog.description.value = img.description || ''; dialog.filename.textContent = img.filename_orig;
                dialog.size.textContent = util.formatBytes(img.size_orig); dialog.date.textContent = util.formatDate(img.created_at);
                dialog.dialog.showModal();
            }
            if (deleteBtn && confirm('确定永久删除这张图片吗?')) { api(`/api/admin/images/${deleteBtn.dataset.id}`, { method: 'DELETE' }).then(res => res && (util.showToast('图片已删除'), loadImages(), loadCategories())); }
        });
        
        elements.editDialog.form.addEventListener('submit', async e => {
            e.preventDefault(); const dialog = elements.editDialog;
            const body = JSON.stringify({ category_id: dialog.category.value, description: dialog.description.value });
            if (await api(`/api/admin/images/${dialog.id.value}`, {method: 'PUT', headers: {'Content-Type': 'application/json'}, body})) { util.showToast('更新成功'); dialog.dialog.close(); loadImages(); }
        });

        elements.categoryMgmt.addBtn.addEventListener('click', async () => { const name = elements.categoryMgmt.newName.value.trim(); if (name) { if(await api('/api/admin/categories', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({name})})) { elements.categoryMgmt.newName.value = ''; await loadCategories(); } } });
        elements.editDialog.cancelBtn.addEventListener('click', () => elements.editDialog.dialog.close());
        elements.searchInput.addEventListener('input', util.debounce(() => { state.search = elements.searchInput.value.trim(); state.currentPage = 1; loadImages(); }, 500));
        elements.pagination.addEventListener('click', e => { const button = e.target.closest('button'); if (button && !button.disabled) { state.currentPage = Number(button.dataset.page); loadImages(); } });
    };
...
show_menu() {
    clear
    echo -e "${BLUE}======================================================${NC}"
    echo -e "${BLUE}      图片画廊 - 极致优化版 (v8.5)      ${NC}"
    echo -e "${BLUE}======================================================${NC}"
    ...
    if pm2 info "$APP_NAME" &> /dev/null && [ "$(pm2 info "$APP_NAME" | grep 'status' | awk '{print $4}')" == "online" ]; then
        ...
        if [ -f "${INSTALL_DIR}/.env" ]; then
            ...
            printf "   %-15s %s\n" "├─ 前台地址:" "${GREEN}http://${SERVER_IP}:${PORT}${NC}"
            printf "   %-15s %s\n" "└─ 后台地址:" "${GREEN}http://${SERVER_IP}:${PORT}/admin${NC}"
        fi
    else
        echo -e " 应用状态: ${RED}已停止${NC}"
    fi
    echo -e "${BLUE}------------------------------------------------------${NC}"
    printf " %-33s %-30s\n" " 1. 安装或修复应用 (首次使用)" " 6. 修改账号和密码"
    printf " %-33s %-30s\n" " 2. 启动应用" " 7. 备份数据"
    printf " %-33s %-30s\n" " 3. 停止应用" " 8. 恢复数据"
    printf " %-33s %-30s\n" " 4. 重启应用" " 9. ${RED}彻底卸载应用${NC}"
    printf " %-33s\n" " 5. 查看实时日志"
    printf " %-33s\n" " 0. 退出"
    echo -e "${BLUE}------------------------------------------------------${NC}"
