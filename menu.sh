...
# =================================================================
#      图片画廊 企业级重构版 - 一体化部署与管理脚本 (v8.0)
#
#   作者: 编码助手
#   功能: 重构核心架构，引入专业级特性，如SQLite, JWT, 图像优化等。
#   手机/PC 全平台响应式支持。
# =================================================================
...
function initDB() {
    ...
    if (!uncategorized) {
        ...
        console.log('"未分类" category created.');
    }
}
...
router.post('/login', loginValidation, async (req, res) => {
    ...
    if (!errors.isEmpty()) {
        ...
        return res.redirect(`/login.html?error=${encodeURIComponent(message)}`);
    }
...
EOF
    ...
    mkdir -p "${INSTALL_DIR}/routes/api"
    echo "--> 正在生成API子模块 routes/api/categories.js..."
cat << 'EOF' > routes/api/categories.js
const { db } = require('../../data/database.js');
const { body, validationResult } = require('express-validator');

// GET /api/categories/public - 公开分类列表（只显示有图片的分类）
exports.getPublicCategories = (req, res) => {
    try {
        const uncategorizedIdQuery = db.prepare(`SELECT id FROM categories WHERE name = '未分类'`).get();
        const uncategorizedId = uncategorizedIdQuery ? uncategorizedIdQuery.id : -1;
        
        const query = `
            SELECT c.id, c.name FROM categories c
            WHERE c.id IN (SELECT DISTINCT category_id FROM images)
            OR c.id = ?
            ORDER BY CASE WHEN c.name = '未分类' THEN 0 ELSE 1 END, c.name COLLATE NOCASE
        `;
        const categories = db.prepare(query).all(uncategorizedId);
        res.json(categories);
    } catch (error) {
        console.error(error);
...
// GET /api/admin/categories - 后台全部分类列表
exports.getAllCategories = (req, res) => {
    try {
        const categories = db.prepare(`SELECT * FROM categories ORDER BY CASE WHEN name = '未分类' THEN 0 ELSE 1 END, name COLLATE NOCASE`).all();
        res.json(categories);
    } catch (error) {
        console.error(error);
...
exports.updateCategory = [
    ...
    (req, res) => {
        ...
        const catToUpdate = db.prepare('SELECT name FROM categories WHERE id = ?').get(id);
        if (!catToUpdate) return res.status(404).json({ message: 'Category not found' });
        if (catToUpdate.name === '未分类') return res.status(403).json({ message: 'Cannot rename "未分类"' });
...
exports.deleteCategory = (req, res) => {
    const { id } = req.params;
    const catToDelete = db.prepare('SELECT name FROM categories WHERE id = ?').get(id);

    if (!catToDelete) return res.status(404).json({ message: 'Category not found' });
    if (catToDelete.name === '未分类') return res.status(403).json({ message: 'Cannot delete "未分类"' });

    try {
        const uncategorized = db.prepare(`SELECT id FROM categories WHERE name = '未分类'`).get();
        if (!uncategorized) throw new Error('"未分类" category not found for migration');

        db.transaction(() => {
            db.prepare('UPDATE images SET category_id = ? WHERE category_id = ?').run(uncategorized.id, id);
            db.prepare('DELETE FROM categories WHERE id = ?').run(id);
        })();
        res.status(200).json({ message: 'Category deleted and images moved to "未分类"' });
    } catch (error) {
        console.error(error);
...
const getBaseQuery = () => `
    SELECT
        ...
        i.path_orig, i.path_display, i.path_thumb,
        i.filename_orig, i.size_orig,
...
const processImage = async (filePath) => {
    ...
    const displayPath = path.join(UPLOADS_DIR, `${nameWithoutExt}.webp`);
    const thumbPath = path.join(UPLOADS_DIR, `thumb-${nameWithoutExt}.webp`);
    ...
    return {
        display: `/uploads/${path.basename(displayPath)}`,
...
exports.getPublicImages = (req, res) => {
    ...
    if (search) {
        whereClauses.push("(i.description LIKE ? OR i.filename_orig LIKE ?)");
        params.push(`%${search}%`, `%${search}%`);
    }

    const where = whereClauses.length > 0 ? `WHERE ${whereClauses.join(' AND ')}` : '';
    const orderBy = category === 'random' ? 'ORDER BY RANDOM()' : 'ORDER BY i.created_at DESC';

    try {
        const countQuery = `SELECT COUNT(*) as total FROM images i JOIN categories c ON i.category_id = c.id ${where}`;
        const total = db.prepare(countQuery).get(params).total;

        const imagesQuery = `${getBaseQuery()} ${where} ${orderBy} LIMIT ? OFFSET ?`;
...
// --- Admin Routes ---
exports.getAdminImages = this.getPublicImages; // Reuse logic for admin panel viewing
...
exports.uploadImage = (req, res) => {
    upload(req, res, async (err) => {
        ...
        if (!category_id) {
             // Clean up uploaded file if category is missing
            fs.unlinkSync(req.file.path);
            return res.status(400).json({ message: 'Category is required.' });
        }

        try {
            const { display, thumb } = await processImage(req.file.path);
            const imageId = uuidv4();
            
            const info = db.prepare(`
                INSERT INTO images (id, category_id, description, filename_orig, path_orig, path_display, path_thumb, size_orig)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            `).run(
                imageId,
                category_id,
            ...
            res.status(201).json({ message: 'Upload successful', imageId });
...
exports.updateImage = [
    ...
    (req, res) => {
        ...
        const { category_id, description } = req.body;
        try {
            const result = db.prepare('UPDATE images SET category_id = ?, description = ? WHERE id = ?')
                           .run(category_id, description, id);
...
exports.deleteImage = (req, res) => {
    const { id } = req.params;
    try {
        const image = db.prepare('SELECT path_orig, path_display, path_thumb FROM images WHERE id = ?').get(id);
        ...
        if (result.changes > 0) {
            ...
            [image.path_orig, image.path_display, image.path_thumb].forEach(p => {
                if (p) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const loadImages = async () => {
        ...
        if (data.images.length === 0 && state.currentPage === 1) {
            loader.textContent = '没有找到图片。';
        } else {
            loader.style.display = 'none';
        }
    ...
    const appendImages = (images) => {
        ...
        images.forEach(data => {
            ...
            item.dataset.index = state.galleryItems.length;
            
            const img = document.createElement('img');
            // Lazy loading
            img.src = "data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7";
            img.dataset.src = data.path_thumb;
    ...
    galleryContainer.addEventListener('click', (e) => {
        ...
        if (item) {
            state.currentLightboxIndex = parseInt(item.dataset.index);
            updateLightbox();
    ...
    const updateLightbox = () => {
        ...
        lightboxImage.src = currentItem.path_display; // Load full version
        lightboxImage.alt = currentItem.description;
        lbDownload.href = currentItem.path_orig;
        lbCounter.textContent = `${state.currentLightboxIndex + 1} / ${state.galleryItems.length}`;
    };

    const showPrevImage = () => {
        state.currentLightboxIndex = (state.currentLightboxIndex - 1 + state.galleryItems.length) % state.galleryItems.length;
        updateLightbox();
    };

    const showNextImage = () => {
        state.currentLightboxIndex = (state.currentLightboxIndex + 1) % state.galleryItems.length;
        updateLightbox();
    };
    ...
    // Scroll handling for infinite scroll and header/back-to-top button
...
<html lang="zh-CN">
<head>
    <meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>后台管理 - 图片画廊</title><script src="https://cdn.tailwindcss.com"></script><style>body{background-color:#f0fdf4}dialog::backdrop{background-color:rgba(0,0,0,0.5)}.category-item.active{background-color:#dcfce7;font-weight:bold}</style>
</head>
<body class="antialiased text-green-900">
    <header class="bg-white shadow-md p-4 flex justify-between items-center sticky top-0 z-20">
        <h1 class="text-xl sm:text-2xl font-bold text-green-900">内容管理</h1>
        <div><a href="/" target="_blank" class="text-green-600 hover:text-green-800 mr-2 sm:mr-4">查看前台</a><a href="/api/auth/logout" class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-3 sm:px-4 rounded transition-colors text-sm sm:text-base">退出</a></div>
    </header>

    <main class="p-4 sm:p-6 lg:p-8">
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
            <div class="lg:col-span-4 xl:col-span-3 space-y-8">
                <section class="bg-white p-6 rounded-lg shadow-md"><h2 class="text-xl font-semibold mb-4">上传新图片</h2>
                    <form id="upload-form" class="space-y-4">
                        <label for="image-input" class="w-full flex flex-col items-center justify-center p-4 border-2 border-dashed border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50"><svg class="w-8 h-8 mb-2 text-gray-500" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 16"><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 13h3a3 3 0 0 0 0-6h-.025A5.56 5.56 0 0 0 16 6.5 5.5 5.5 0 0 0 5.207 5.021C5.137 5.017 5.071 5 5 5a4 4 0 0 0 0 8h2.167M10 15V6m0 0L8 8m2-2 2 2"/></svg><p class="text-sm text-gray-500"><span class="font-semibold">点击选择文件</span></p><input id="image-input" name="image" type="file" class="hidden" required accept="image/png, image/jpeg, image/gif"/>
                        </label>
                        <div id="file-info-wrapper" class="mt-2 text-xs text-gray-500 hidden"><p id="file-name-info"></p><p id="file-size-info"></p></div>
                        <div><label for="category-select" class="block text-sm font-medium mb-1">图片分类</label><select name="category_id" id="category-select" required class="w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-green-500"></select></div>
                        ...
                        <button type="submit" id="upload-btn" class="w-full bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded-lg transition-colors disabled:bg-gray-400">上传图片</button>
                    </form>
                </section>
                <section class="bg-white p-6 rounded-lg shadow-md"><h2 class="text-xl font-semibold mb-4">分类管理</h2>
                    <div class="flex items-center space-x-2 mb-4"><input type="text" id="new-category-name" placeholder="输入新分类名称" class="w-full border rounded px-3 py-2"><button id="add-category-btn" class="flex-shrink-0 bg-green-500 hover:bg-green-600 text-white font-bold w-9 h-9 rounded-full flex items-center justify-center text-xl">+</button></div>
                    <div id="category-management-list" class="space-y-2 max-h-60 overflow-y-auto"></div>
                </section>
            </div>
            <section class="bg-white p-6 rounded-lg shadow-md lg:col-span-8 xl:col-span-9"><div class="flex flex-wrap justify-between items-center gap-4 mb-4">
                    <h2 class="text-xl font-semibold">已上传图片 <span id="image-count" class="text-base text-gray-500 font-normal"></span></h2>
    ...
    const showToast = (message, isError = false) => {
        ...
        toast.className = `fixed top-5 right-5 p-4 rounded-lg shadow-lg text-white ${isError ? 'bg-red-500' : 'bg-green-600'}`;
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const elements = {
            ...
            fileInfo: { wrapper: document.getElementById('file-info-wrapper'), name: document.getElementById('file-name-info'), size: document.getElementById('file-size-info') },
            ...
            editDialog: { dialog: document.getElementById('edit-image-dialog'), form: document.getElementById('edit-image-form'), id: document.getElementById('edit-id'), preview: document.getElementById('edit-preview'), category: document.getElementById('edit-category-select'), description: document.getElementById('edit-description'), cancelBtn: document.getElementById('cancel-edit-btn') },
        };
        
        let state = { categories: [], currentPage: 1, totalPages: 1, filterCategory: 'all', search: '' };
        ...
        const api = async (url, options = {}) => {
            try {
                ...
                if (!response.ok) { showToast(data.message || '操作失败', true); return null; }
                return data;
            } catch (error) {
                showToast('网络错误', true);
                return null;
        ...
        const populateCategoryDropdowns = () => {
            [elements.categorySelect, elements.editDialog.category].forEach(select => {
                select.innerHTML = '';
        ...
        const loadImages = async () => {
            const params = new URLSearchParams({ page: state.currentPage, limit: 10, category: state.filterCategory, search: state.search });
            const data = await api(`/api/admin/images?${params.toString()}`);
            if (data) {
                renderImageList(data.images);
                renderPagination(data.currentPage, data.totalPages);
                elements.imageCount.textContent = `(${data.images.length} of ${data.totalPages * 10} approx.)`; // Approximation
            }
        };

        const renderImageList = (images) => {
            elements.imageList.innerHTML = images.map(img => `
        ...
        const renderPagination = (currentPage, totalPages) => {
            ...
            let buttons = [];
            // Prev button
            buttons.push(`<button data-page="${currentPage - 1}" ${currentPage === 1 ? 'disabled' : ''} class="px-3 py-1 rounded bg-gray-200 disabled:opacity-50">&laquo;</button>`);
            // Page numbers
            for (let i = 1; i <= totalPages; i++) {
                if (i === currentPage) {
                    buttons.push(`<button data-page="${i}" class="px-3 py-1 rounded bg-green-600 text-white">${i}</button>`);
                } else if (i <= 2 || i >= totalPages - 1 || Math.abs(i - currentPage) <= 1) {
                    buttons.push(`<button data-page="${i}" class="px-3 py-1 rounded bg-gray-200">${i}</button>`);
                } else if (Math.abs(i - currentPage) === 2) {
                     buttons.push(`<span>...</span>`);
                }
            }
            // Next button
            buttons.push(`<button data-page="${currentPage + 1}" ${currentPage === totalPages ? 'disabled' : ''} class="px-3 py-1 rounded bg-gray-200 disabled:opacity-50">&raquo;</button>`);
        ...
        elements.imageInput.addEventListener('change', e => {
            ...
            if (file) {
                elements.fileInfo.name.textContent = file.name;
                elements.fileInfo.size.textContent = `${(file.size / 1024).toFixed(1)} KB`;
                elements.fileInfo.wrapper.classList.remove('hidden');
            } else {
                elements.fileInfo.wrapper.classList.add('hidden');
            }
        });
        ...
        elements.uploadForm.addEventListener('submit', async e => {
            ...
            const res = await api('/api/admin/images/upload', { method: 'POST', body: formData });
            if (res) {
                showToast('上传成功!');
            ...
            elements.uploadBtn.textContent = '上传图片';
        });
        
        elements.categoryMgmt.addBtn.addEventListener('click', async () => {
            const name = elements.categoryMgmt.newName.value.trim();
            if (name && await api('/api/admin/categories', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ name }) })) {
                elements.categoryMgmt.newName.value = '';
                loadCategories();
            }
        });
        ...
        elements.categoryMgmt.list.addEventListener('click', async e => {
            ...
            if (target.classList.contains('delete-cat-btn')) {
                ...
                if (newName && newName.trim() !== name) {
                    if (await api(`/api/admin/categories/${id}`, { method: 'PUT', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ name: newName.trim() }) })) {
                         showToast('分类已重命名');
                         loadCategories();
                    }
                }
                ...
                if(state.filterCategory != 'all') item.classList.add('active');
                loadImages();
            }
        });
        ...
        elements.imageList.addEventListener('click', async e => {
            const editBtn = e.target.closest('.edit-btn');
            const deleteBtn = e.target.closest('.delete-btn');
            if(editBtn) {
                const img = JSON.parse(editBtn.dataset.image);
                const dialog = elements.editDialog;
                dialog.id.value = img.id;
                dialog.preview.src = img.path_thumb;
                dialog.category.value = img.category_id;
                dialog.description.value = img.description || '';
                dialog.dialog.showModal();
            }
            if(deleteBtn) {
                const id = deleteBtn.dataset.id;
                if (confirm('确定永久删除这张图片吗?')) {
                    if(await api(`/api/admin/images/${id}`, { method: 'DELETE' })) {
                        showToast('图片已删除');
                        loadImages();
                    }
                }
        ...
        elements.editDialog.form.addEventListener('submit', async e => {
            ...
            const body = JSON.stringify({
                category_id: dialog.category.value,
                description: dialog.description.value
            });
            if(await api(`/api/admin/images/${id}`, {method: 'PUT', headers: {'Content-Type': 'application/json'}, body})) {
                showToast('更新成功');
                dialog.dialog.close();
                loadImages();
            }
...
install_app() {
    ...
    if ! command -v node > /dev/null || ! command -v npm > /dev/null; then
        ...
        # Add robust installation for Debian/Ubuntu and RHEL/CentOS
        if [ -f /etc/debian_version ]; then
            apt-get update -y && apt-get install -y nodejs npm
        elif [ -f /etc/redhat-release ]; then
            yum install -y nodejs npm
        else
            echo -e "${RED}无法确定操作系统类型，请手动安装 Node.js (v16+) 和 npm。${NC}"
            exit 1
        fi
    ...
    read -sp "请输入后台管理员密码 (必填): " password
    ...
    echo "ADMIN_PASSWORD=${password}" >> .env # Temporarily store plain password

    echo "--> 正在安装项目依赖 (npm install)..."
    npm install
    
    echo "--> 正在初始化数据库并设置管理员账户..."
    npm run setup
    
    # Remove plain password from .env after setup
...
start_app() {
    ...
    if ! [ -f ".env" ]; then
        echo -e "${RED}错误: .env 文件不存在。请先运行安装程序。${NC}"; return 1;
    fi;
...
manage_credentials() {
    ...
    if ! [ -f ".env" ]; then
        echo -e "${RED}错误: .env 文件不存在。请先安装应用。${NC}"; return 1;
    fi;
    ...
    read -sp "请输入新的密码 (必填): " new_password;
    echo ""
    
    if [ -z "$new_password" ]; then
        echo -e "${RED}密码不能为空！操作取消。${NC}"; return 1;
    fi;

    local FINAL_USER=${new_username:-$CURRENT_USER}
    
    # Update .env temporarily for setup script
    ...
    # Clean up temp entries from .env
    sed -i '/^ADMIN_USERNAME=/d' .env
    sed -i '/^ADMIN_PASSWORD=/d' .env
    # Add back the correct username
    echo "ADMIN_USERNAME=${FINAL_USER}" >> .env
...
backup_app() {
    ...
    if [ $? -eq 0 ]; then
        echo -e "${GREEN}备份成功! 文件保存在: ${BACKUP_FILE}${NC}"
    else
        echo -e "${RED}备份失败!${NC}"
    fi
}
...
restore_app() {
    ...
    if [ ! -d "${BACKUP_DIR}" ] || [ -z "$(ls -A ${BACKUP_DIR})" ]; then
        echo -e "${RED}没有找到备份文件或备份目录! (${BACKUP_DIR})${NC}"
        return 1
    fi
    ...
    tar -xzf "${backup_file}" -C "/" # Extract to root to preserve paths like ./data/...
...
show_menu() {
    clear
    echo -e "${BLUE}======================================================${NC}"
    echo -e "${BLUE}      图片画廊 - 企业级重构版 (v8.0)      ${NC}"
    echo -e "${BLUE}======================================================${NC}"
